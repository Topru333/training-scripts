...
class LoraTrainerRunner():
    ...
    def submit(self, setup_json):
        ...
        max_grad_norm_command = ""
        if setup_json["max_grad_norm"] > 0:
            max_grad_norm_command = f'--max_grad_norm={setup_json["max_grad_norm"]}'
        ...
        match setup_json['lora_module']:
            case 'LoRA-C3Lier':
                lora_module_command = f'--network_module=networks.lora --network_args "conv_dim={setup_json["conv_dim"]}" "conv_alpha={setup_json["conv_alpha"]}"'
            case 'Locon':
                lora_module_command = f'--network_module=lycoris.kohya --network_args "conv_dim={setup_json["conv_dim"]}" "conv_alpha={setup_json["conv_alpha"]}" "algo=locon"'
            case 'Loha':
                lora_module_command = f'--network_module=lycoris.kohya --network_args "conv_dim={setup_json["conv_dim"]}" "conv_alpha={setup_json["conv_alpha"]}" "algo=loha"'
            case 'Lokr':
                lora_module_command = f'--network_module=lycoris.kohya --network_args "conv_dim={setup_json["conv_dim"]}" "conv_alpha={setup_json["conv_alpha"]}" "algo=lokr"'
            case 'Ia3':
                lora_module_command = f'--network_module=lycoris.kohya --network_args "conv_dim={setup_json["conv_dim"]}" "conv_alpha={setup_json["conv_alpha"]}" "algo=ia3"'
            case 'Lora':
                lora_module_command = "--network_module=networks.lora"
            case 'Lora-SDXL':
                lora_module_command = "--network_module=networks.lora"
                script_name = "sdxl_train_network.py"
            case _:
                raise Exception("Unsupported lora module")
        ...
        {use_mem_eff_attn_command} \
        {generate_samples_command} \
        {"--no_half_vae" if setup_json['lora_module'] == "Lora-SDXL" else ""}"""
