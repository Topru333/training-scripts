...
import os
import json
from dreambooth_trainer_runner import DreamboothTrainerRunner

cache_dir = os.path.join(os.path.expanduser("~"), ".cache")
training_runner = DreamboothTrainerRunner(last_setup_path=cache_dir, notification_fn=lambda notification: print(notification))

last_lora_training_setup = os.path.join(cache_dir, "last_training_db_model.json")
...
def process(
    ...
    setup_json = {
        ...
        'reg_dataset_path': reg_data_path_textbox,
...
if os.path.exists(last_lora_training_setup):
    ...
    print("Last dreambooth model setup does not exist.")
...
def training_gui():
    with gr.Blocks() as app:
        ...
        reg_dataset_path_textbox = gr.Textbox(label="Reg dataset path:", lines=1, interactive=True, value=gradio_values.get("reg_dataset_path", ""), visible=True)
        ...
        gradient_accumulation_steps_slider = gr.Slider(1, 20, value=gradio_values.get("gradient_accumulation_steps", 1), step=1, label="Gradient accumulation steps:", interactive=True)
        ...
        chackbox_settings_group = gr.CheckboxGroup(
